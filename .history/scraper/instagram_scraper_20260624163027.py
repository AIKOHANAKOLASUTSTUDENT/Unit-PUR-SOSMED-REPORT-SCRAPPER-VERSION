...
class InstagramScraper:
    ...
    def _scrape_meta_api(self, url: str, route_type: str = "Post") -> Dict[str, Any]:
        ...
        # Fetch account media pages to find matching shortcode (paginate)
        media_id = None
        media_item = None
        page_count = 0
        ...
        while page_count < 10 and not media_id:
            try:
                ...
                self.logger.debug(f"Page {page_count+1}: found shortcodes: {shortcodes}")
        ...
        try:
            ...
            params = {
                "metric": "impressions,reach,saved,shares,plays,likes,comments",
                "period": "lifetime",
                "access_token": META_ACCESS_TOKEN,
            }
            ...
            for insight in data.get("data", []):
                ...
                # insights can be returned as dicts with values list
        ...
        # Build record
        # Prefer insights for likes/comments/views when available, fallback to media fields
        likes = insights.get("likes", media_item.get("like_count", "N/A"))
        comments = insights.get("comments", media_item.get("comments_count", "N/A"))
        # For video content, 'plays' is a better metric than 'video_views'
        ...
        # Detailed debug log matching Apify format for easy comparison
        self.logger.debug(
            f"Meta API raw for {url}: likes={likes} comments={comments} views={views} saves={saves} shares={shares} date={date} type={media_type}"
        )
