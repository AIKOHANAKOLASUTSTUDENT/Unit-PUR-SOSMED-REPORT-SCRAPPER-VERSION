...
class InstagramScraper:
    ...
    def _scrape_one(self, url: str) -> Dict[str, Any]:
        ...
        if "/reel/" in lower_url:
            self.logger.info(f"Routing as REEL → instaloader: {url}")
            if HAS_INSTALOADER:
                return self._scrape_instaloader(url)
            self.logger.warning("Instaloader not available, trying Meta API")
            return self._scrape_meta_api(url) if META_ACCESS_TOKEN else self._empty_record(url)

        elif "/p/" in lower_url:
            self.logger.info(f"Routing as POST → Meta API: {url}")
            if META_ACCESS_TOKEN:
                return self._scrape_meta_api(url)
            elif APIFY_TOKEN:
                return self._scrape_apify(url)
            else:
                self.logger.warning(f"No scraper available for Post URL: {url}")
