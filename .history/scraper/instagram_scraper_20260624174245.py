...
class InstagramScraper:
    ...
    def _scrape_meta_api(self, url: str, route_type: str = "Post") -> Dict[str, Any]:
        ...
        # Prefer play metrics for views, then video_views, then known media fields.
        views = insights.get("plays", None)
        if views in [None, "N/A", 0]:
            views = insights.get("video_views", None)
        if views in [None, "N/A", 0]:
            views = media_item.get("video_view_count", None)
        if views in [None, "N/A", 0]:
            views = media_item.get("video_views", None)
        if views in [None, "N/A", 0]:
            views = media_item.get("view_count", "N/A")
