...
if scrape_clicked and len(parsed_urls) > 0:
    try:
        ...
        status_text.info(f"🎬 Phase 1: Scraping {len(reel_urls)} Reels...")
        for idx, url in enumerate(reel_urls):
            record = scraper._scrape_one_instaloader(url)
            raw_records.append(record)
            progress_bar.progress((idx + 1) / max(1, total_urls))

        status_text.info(f"🖼️ Phase 2: Scraping {len(post_urls)} Posts (may return N/A)...")
        for idx, url in enumerate(post_urls):
            record = scraper._scrape_one_apify(url)
            raw_records.append(record)
...
if st.session_state["has_results"] and len(st.session_state["results"]) > 0:
    ...
    column_order = [
        ...
        "saves", "shares", "caption", "ingestion_timestamp"
    ]
    available_columns = [col for col in column_order if col in df.columns]
    df = df[available_columns]
    
    # Display an informational banner for result types
    st.info("✅ Reel: data lengkap tersedia  |  ⚠️ Post foto: coming soon via Meta API")
