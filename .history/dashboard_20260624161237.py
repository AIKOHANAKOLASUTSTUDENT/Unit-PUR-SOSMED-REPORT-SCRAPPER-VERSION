...
if scrape_clicked and len(parsed_urls) > 0:
    try:
        ...
        status_text.info(
            f"🎬 Phase 1: Scraping {len(reel_urls)} Reels..."
            + (" (Meta API enabled)" if has_meta else "")
        )
        ...
        status_text.info(
            f"🖼️ Phase 2: Scraping {len(post_urls)} Posts..."
            + (" (Meta API enabled)" if has_meta else " (Apify fallback if available)")
        )
